...
const ShaderBackground: React.FC<Props> = ({ opacity = 0.55 }) => {
  ...
  useEffect(() => {
    ...
    // Same shader logic as provided in the prompt
  ...
  return (
    <>
      <canvas 
        ref={canvasRef} 
        className="fixed top-0 left-0 w-full h-full block z-0 pointer-events-none transition-opacity duration-500 ease-out"
        style={{ opacity }}
      />
      {/* Vignette overlay */}
      <div 
        className="fixed inset-0 z-[1] pointer-events-none transition-opacity duration-500 ease-out mix-blend-screen"
        style={{ 
          opacity,
          background: `
            radial-gradient(900px 600px at 34% 52%, rgba(0,255,255,.10), transparent 60%),
            radial-gradient(900px 600px at 70% 60%, rgba(255,40,120,.08), transparent 65%),
            radial-gradient(1200px 900px at 50% 40%, rgba(0,0,0,.0), rgba(0,0,0,.55))
          ` 
        }}
      ></div>
    </>
